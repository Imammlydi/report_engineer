...
const ChartPekerja = ({ idReport, tokens, durasikerja, fetchDurasi }) => {
    ...
    const categories = durasikerja.map((item) => item.type_of_worker);
    const series = [
        {
            name: "Length",
    ...
    return (
        <>
            <div className="mt-2 rounded-md border p-2 shadow-lg">
                <p className="p-2 font-Bitter font-bold">Pekerja</p>
                <Chart
                    options={options}
                    series={series}
                    type="bar"
