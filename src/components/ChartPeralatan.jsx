...
const ChartPerlatan = ({ idReport, tokens, durasikerja, fetchDurasi }) => {
    ...
    const categories = durasikerja.map((item) => item.name);
    const series = [
        {
            name: "Length",
            data: durasikerja.map((item) => item.qty),
        },
    ];
    ...
    return (
        <>
            <div className="mt-2 rounded-md border p-2 shadow-lg">
                <p className="p-2 font-Bitter font-bold">
                    Peralatan di Sekitar Kerja
                </p>
                <Chart
                    options={options}
                    series={series}
